...
class ExcelOps:    
    def create_hive_table(table_name, schema):
        ...
        hive_schema = ExcelOps.get_hive_table_schema(table_name)
    ...
    def insert_data_into_hive_table(table_name, data):
        ...
        for index, row in data.iterrows():
            values = ', '.join([ExcelOps.clean_value(value) for value in row.values])
...
def excel_to_hive(excel_file):
    ...
    for sheet_name in excel_data.sheet_names:
        ...
        ExcelOps.create_hive_table(table_name, column_types)
        ExcelOps.insert_data_into_hive_table(table_name, df)
        hive_schema = ExcelOps.get_hive_table_schema(table_name)
        print(f"\n--->Hive Table Schema for {table_name}:")
        for column, data_type in hive_schema.items():
            print(f"{column}: {data_type}")
        print("-----------------------------------")
        print(f"Table '{table_name}' is created successfully in the Hive database.")


def process_new_excel_files(**kwargs):
    path_to_watch = '/opt/airflow/dags/data/'
    excel_files = glob.glob(os.path.join(path_to_watch, '*.xlsx'))
