...
def check_table_metadata_quality(table_metadata):
    ...
    if table_guid:
        if table_guid in guid_list_tab:
            ...
            has_warnings = True
            table_metadata["Unique_GUID"] = False  # Update the entry directly
            anomaly=+1
        else:
            guid_list_tab.append(table_guid)
            table_metadata["Unique_GUID"] = True  # Indicate GUID is unique
    else:
    ...
    if expected_num_attributes != actual_num_attributes:
        ...
        has_warnings = True
        table_metadata["Valid_Number_of_Attributes"] = True
    ...
    if invalid_attrs:
        ...
        table_metadata['HasInvalidAttributes'] = True  # Flag indicating there are invalid attributes
    else:
